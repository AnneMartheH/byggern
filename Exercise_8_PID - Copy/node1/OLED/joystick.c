...
menu_ptr menu_add(menu_ptr parent, char * text, void (*function)()){
	menu_ptr subMenu = malloc(sizeof(menu_t)); // empty dub menu element
	subMenu ->title = text; // title
	subMenu ->parent = parent; //pointer to parent
	subMenu ->function = function; //pointer to function // remove ()??
